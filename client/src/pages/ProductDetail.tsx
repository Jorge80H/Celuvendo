  ...
  const handleAddToCart = async () => {
    ...
    if (product.colors && product.colors.length > 0 && !selectedColor) {
      toast({
        ...
        description: "Por favor selecciona el color del producto antes de agregarlo al carrito",
        variant: "destructive",
      });
      return;
    }
    ...
    setIsAdding(true);
    try {
      await addToCartInstant(product.id, 1, selectedColor || undefined);
      toast({
        title: "Producto agregado",
        description: `${product.name}${selectedColor ? ` - ${selectedColor}` : ''} fue agregado al carrito`,
      });
    } catch (error) {
      toast({
        title: "Error",
              ...
              {product.colors && product.colors.length > 0 && (
                <div>
                  <p className="text-sm font-medium mb-2">
                    Colores disponibles:
                    {selectedColor && <span className="text-primary ml-2">({selectedColor})</span>}
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {product.colors.map((color: any) => {
                      ...
                      return (
                        <div
                          key={color.name}
                          className={`flex items-center gap-2 px-3 py-2 border-2 rounded-md cursor-pointer transition-all duration-200 ${
                            isSelected
